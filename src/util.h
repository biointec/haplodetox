...
class BinAssignment : public std::vector<int>
{
private:
...
public:
         ...
         */
        BinAssignment(int size) :
        std::vector<int>(size, 0), card(std::vector<int>(size,2)) {};
...
public:
        ...
        /**
         * Compute the relative distance between two vectors of numbers
         * @param a vector one
         * @param b vector two
         * @return The maximum relative distance, computed element-wise
         */
        static double relDiff(const std::vector<double>& a,
                              const std::vector<double>& b) {
                assert(a.size() == b.size());
                double res = 0.0;
                for (size_t i = 0; i < a.size(); i++)
                        res = std::max<double>(res, relDiff(a[i], b[i]));
                return res;
        }
        ...
        static int fitTruncNegBinomEM(std::map<unsigned int, double>& data,
                                      ...
                                      double& w, double epsilon, int maxIter, bool fixed = false);
        ...
        /**
         * Computer a^b where a and be are integral types
         * @param a base
         * @param b exponent
         * @return a^b
         */
        static size_t pow(size_t a, size_t b) {
                return round(std::pow(a, b));
        }

        /**
         * Compute the Log-Likelihood for the mixture model fitted
         * with the function haplotypeNbinomialMixtureEM
         * @param hist the histogram to which the model was fitted
         * @param mu list of negative binomial means of the model components
         * @param f list of negative binomial overdispersion factors
         * @param MC matrix of model weights
         * @return the log-likelihood value
         */
        static double initEMLL(const std::map<unsigned int, double>& hist,
        ...
        /**
         * Given a log-likelihood score and the necessary model properties,
         * calculate the AIC score
         * @param logL Log-Likelihood score value
         * @param nstrains number of strains in the model
         * @param maxM maximum multiplicity for which parameters are calculated explicitly
         * @return the AIC score value
         */
        static double haplotypeAIC(const double logL, int nstrains, int maxM) {
                double AIC = -2*logL;
                // err-param + lambda_i + odf + weights
                int nparam = 3 + nstrains + 1 + (pow(2,nstrains) - 1)*maxM;
                AIC += 2 * nparam;
                return AIC;
        }
         ...
         * @param nstrains number of strains in the model
         * @param maxM maximum multiplicity for which parameters are calculated explicitly
         * @return the BIC score value
         */
        static double haplotypeBIC(const double logL, int nstrains, int maxM, int n) {
                double BIC = -2*logL;
                // err-param + lambda_i + odf + weights
                int nparam = 3 + nstrains + 1 + (pow(2,nstrains) - 1)*maxM;
                BIC += log(n) * nparam;
        ...
        /**
         * Given the model parameters (of the NB-mixture) and the observed histogram,
         * calculate the Completed Log Likelihood score
         * @param hist the histogram to which the model was fitted
         * @param mu list of negative binomial means of the model components
         * @param f list of negative binomial overdispersion factors
         * @param MC matrix of model weights
         * @return the completed log-likelihood value
         */
        static double haplotypeCLC(const std::map<unsigned int, double>& hist,
                                   const std::vector<double>& mu,
                                   const std::vector<double> &f,
                                   const std::vector<std::vector<double>>& MC);
	...
	/**
	 * Basic Gaussian elimination (with row pivot)
	 * to solve the system of equations Ax = b
	 * with A an mxm square matrix and b a 1xm vector
	 * @param A an mxm square matrix (will be altered in place by the computations)
	 * @param b a 1xm vector (will be altered in place by the computations)
	 * @param x the results vector
	 */
	static void gaussianEliminate(std::vector<double>& A,
				      std::vector<double>& b,
			              std::vector<double>& x);
