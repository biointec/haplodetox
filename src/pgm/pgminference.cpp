/******************************************************************************
 *   Copyright (C) 2018 - 2023 Jan Fostier (jan.fostier@ugent.be)             *
...
int PGMInference::findVarToEliminate(const AdjMatrix& adjMatrix, const vector<int>& varToKeep)
{
        // find the random variable that has the fewest neighbors
        unsigned int minNeighbors = adjMatrix.size();
        int varToEliminate = 0;
        ...
        for (const auto& it : adjMatrix) {
                ...
                // WARNING: linear time operation on a vector (size of that vector???)
                if (find(varToKeep.begin(), varToKeep.end(), var) != varToKeep.end())
                        continue;
                                      ...
                                      vector<int>& elimOrder)
{
        ...
        // make sure the target variable is contained in the factorList
        for (int var: varToKeep)
                assert(adjMatrix.find(var) != adjMatrix.end());

        // now find an elimination ordering
        elimOrder.clear();
        elimOrder.reserve(adjMatrix.size() - varToKeep.size());

        size_t largestFactorSize = 1;
        while (adjMatrix.size() > varToKeep.size()) {  // eliminate all but the variables to keep
                // find the best variable to eliminate
                int varToEliminate = findVarToEliminate(adjMatrix, varToKeep);
                elimOrder.push_back(varToEliminate);
...
bool PGMInference::solveVE(FactorList& factorList, const vector<int>& targetVar,
                           size_t& maxFactorSize)
{
        // get out when the factor list is empty
        if (factorList.empty())
...
//        //write factorSize to file TODO remove
//        std::ofstream outfile("factorSize.tsv", std::ios_base::app);
//        outfile << factorSize << endl;
//        outfile.close();
