...
void RefComp::ignoreLowerCase()
{
        for (size_t i = 0; i < sequence.size(); i++) {
                std::transform(sequence[i].begin(), sequence[i].end(), sequence[i].begin(), std::ptr_fun<int, int>(std::toupper));
                ...
                cout << " (" << nonACTG << " non-ACGT, ignored lower case)" << endl;
        }
}
...
void RefComp::getTrueStrainMultiplicity(NodeMap<vector<int>>& nodeMult,
                                        EdgeMap<vector<int>>& edgeMult,
                                        size_t strainID)
{
        // create an empty node and edge map
        //nodeMult = NodeMap<vector<int>>(dBG.getNumValidNodes());
        //edgeMult = EdgeMap<vector<int>>(dBG.getNumValidArcs() / 2);
        ...
        for (const string& seq : sequence) {
                ...
                NodePosPair prev = table.find(it.getKmer());
                if (prev.isValid())
                        nodeMult[abs(prev.getNodeID())][strainID]++;
                ...
                for (it++; it.isValid(); it++) {
                        ...
                        // update kmer counters
                        if (curr.isValid())
                                nodeMult[abs(curr.getNodeID())][strainID]++;

                        // get the arc multiplicity
                        if (dBG.crossesArc(prev, curr)) {
                                // arc from prev to curr
                                EdgeRep er(prev.getNodeID(), curr.getNodeID());
                                edgeMult[er][strainID]++;
                        }
        ...
        for (size_t i = 1; i <= dBG.getNumNodes(); i++) {
                ...
                double ML = node.getMarginalLength();
                nodeMult[i][strainID] = round((double)nodeMult[i][strainID]/ML);
        }
        ...
        // compute the multiplicity for each node TODO
