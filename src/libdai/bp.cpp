...
void BP::setProperties( const PropertySet &opts ) {
    ...
    if ( opts.hasKey("modelcount") )
        props.modelcount = opts.getStringAs<std::string>("modelcount");
    else
        props.modelcount = "0";
...
Real BP::run() {
    ...
    for( ; _iters < props.maxiter && maxDiff > props.tol && (toc() - tic) < props.maxtime; _iters++ ) {
        ...
        for( size_t i = 0; i < nrVars(); ++i ) {
            ...
            }else if (dist( b, _oldBeliefsV[i], DISTLINF ) > props.tol)
                    numNotC++;
        ...
        if( props.forcestop ){
                ...
                if((_iters > 10) && (((double) numNotC / (double) nrVars()) < props.tol))
                        break;
        }
        numVarNonConv_old = numNotC;
    }
    ...
    if( props.verbose >= 1 ) {
        if( maxDiff > props.tol ) {
            if( props.verbose == 1 )
                cerr << endl;
            cerr << name() << "::run:  WARNING: not converged after " << _iters << " passes, " << numVarNonConv_old << " out of " << nrVars() << " variables not converged yet (" << toc() - tic << " seconds)...final maxdiff:" << maxDiff << endl;
        } else {
            if( props.verbose >= 2 )
                cerr << name() << "::run:  ";
