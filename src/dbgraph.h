...
private:
        ...
        std::map<NodeRep, std::vector<int>> trueNodeMult;
        std::map<EdgeRep, std::vector<int>> trueEdgeMult;
...
public:
        ...
        /**
         * Read and store true multiplicities of nodes and arcs
         *
         */
        void readTrueStrainMultiplicities(const Settings& settings, 
                                          int nStrains, bool correct);
        ...
        void writeCytoscapeGraph(const std::string& filename,
                                 ...
                                 const NodeMap<std::vector<int>>& trueNodeStrainMult,
                                 const EdgeMap<std::vector<int>>& trueEdgeStrainMult) const;

         /**
         * Write a Cytoscape graph of the graph in stage 1, containing multiple strains
         * Stage 1: so no estimated multiplicities are needed
         * @param filename Filename of the cytoscape graph
         * @param nodes Nodes in subgraph
         * @param edges Edges in subgraph
         * @param estNodeMult Estimated node multiplicity
         * @param estEdgeMult Estimated edge multiplicity
         * @param trueNodeMult True node multiplicity per strain
         * @param trueEdgeMult True edge multiplicity per strain
         */
        void writeStage1CytoscapeGraph(const std::string& filename,
                                       std::vector<NodeID> nodes,
                                       std::vector<std::pair<NodeID, NodeID>> edges,
                                       const NodeMap<std::vector<int>>& trueNodeStrainMult,
                                       const EdgeMap<std::vector<int>>& trueEdgeStrainMult) const;
                                       
        void writeStage1GFA(const std::string& filename,
                                       std::vector<NodeID> nodes,
                                       std::vector<std::pair<NodeID, NodeID>> edges,
                                       const NodeMap<std::vector<int>>& trueNodeStrainMult,
                                       const EdgeMap<std::vector<int>>& trueEdgeStrainMult) const;
        ...
        /**
         * Get the full directed graph (for e.g. visualisation purposes)
         * @param nodes Vector of nodes in the graph (output)
         * @param edges Vector of edges in the graph (output)
         */
        void getFullDirGraph(std::vector<NodeID>& nodes,
         ...
         * @return A vector of node representations
         */
        std::vector<NodeRep> getLowCovSmallNodes(double threshold, int maxLength) const;
        ...
        /**
         * Get tips with an average coverage below the threshold
         * and marginal length below maxLength
         * @param threshold Coverage threshold
         * @param maxLength maximum marginal length to consider
         * @return A vector of node representations
         */
        std::vector<NodeRep> getLowCovSmallTips(double threshold, int maxLength) const;
        ...
        /**
         * Get evaluation of model scores nodes subset
         * 
         */
        std::vector<NodeRep> getEvalNodes(double min, double max, size_t N, const std::string& filename) const;
        std::vector<NodeRep> getEvalNodes(const std::string& filename) const;
        //TODO much more difficult to predict!
        // std::vector<EdgeRep> getEvalEdges(double min, double max) const;

        /**
         * Get a quick estimate for the k-mer coverage
         * @param errLamda Mean coverage estimate for sequencing error (Poisson)
         * @param p
         */
        double estimateInitKmerCov(double errLambda, double p = 0.01) const;

        /**
         * Get a (rough) estimate for the k-mer coverage model
         * @param settings Const-ref to settings object
         * @return Coverage model
         */
        CovModel getInitCovModel(const Settings& settings, const int nStrains);

        /**
         * Get initial KmerCov estimates for each strain separately
         * @param cutoff Cutoff for low coverage nodes
         * @param nStrains number of strains
         * @param initLambdas KmerCov estimates
         * @param initW Occurences of each peak
         * @param initODF Overdispersion factor
         * @param maxIter Maximum number of iterations for EM
         * @return number of iterations before convergence
         */
        int getInitialLambdaEstimates(double cutoff, int nStrains,
                                      std::vector<double>& initLambdas,
                                      double& lambdaHighM,
                                      std::vector<double>& initW,
                                      double& initODF, double& ODFhighM,
                                      int maxIter) const;
        /**
         * Rescale the initial k-mer counts model weight matrix to the number of nodes in the graph
         * @param nodes Vector with all NodeRep elements
         * @param nodeModel the nodeModel in which the weight matrix has to be adapted
         */                              
        void rescaleKmerToNodeCts(const std::vector<NodeRep>& nodes, CovModel& nodeModel);
         ...
         */
        void getNodeCovHist(std::map<unsigned int, double>& hist) const;
        ...
        /**
         * get a true node multiplicity for a given NodeRep
         */
         std::vector<int> getTrueNodeMult(const NodeRep& n) const {
                return trueNodeMult.empty()? std::vector<int>() : trueNodeMult.at(n);
        }

        /**
         * get a true edge multiplicity for a given EdgeRep
         */
         std::vector<int> getTrueEdgeMult(const EdgeRep& e) const {
                return trueEdgeMult.empty()? std::vector<int>() : trueEdgeMult.at(e);
        }
};
